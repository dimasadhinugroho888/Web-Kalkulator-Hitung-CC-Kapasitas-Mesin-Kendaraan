...
window.addEventListener('load', () => {
  ...
  if(last){
    try{
      ...
      exampleEl.textContent = `${obj.bore} cm × ${obj.stroke} cm × ${obj.silinder} silinder`;
    }catch(e){}
  }
});
...
form.addEventListener('submit', (e) => {
  ...
  if(isNaN(bore) || isNaN(stroke) || isNaN(silinder) || bore <= 0 || stroke <= 0 || silinder <= 0){
    // simple visual shake using GSAP
  ...
  exampleEl.textContent = `${bore} cm × ${stroke} cm × ${silinder} silinder`;
...
// Optional: open repo button (placeholder)
document.getElementById('github-btn').addEventListener('click', (e) => {
  e.preventDefault();
  alert('Silakan upload repo ini ke GitHub dan tautkan di sini jika ingin.');
});
